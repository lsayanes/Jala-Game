...
namespace draw
{
    namespace utils
    {
        ...
        inline void bgraToRgba(unsigned char* pbyBgra, size_t stSize)
        {
        ...
        template <typename T>
        inline void deletePtrVct(T& vct)
        {
            std::for_each(vct.begin(), vct.end(),
                [&](auto it)
                {
                    delete it;
                });
        ...
        [[nodiscard]] inline uint32_t crcI32(uint32_t  ulCRC, const unsigned char *pbyBuffer, unsigned short wDataLen)
        {
            ...
            while (wDataLen--)
                ulCRC = I32Poli04C11DB7_Table[(ulCRC ^ *pbyBuffer++) & 0xFF] ^ (ulCRC >> 8);

            return ulCRC ^ 0xFFFFFFFF;
        }

        template<typename T>
        [[nodiscard]] inline T to32bitData(const unsigned char* pb)
        {
